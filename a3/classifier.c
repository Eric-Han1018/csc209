 ...
 *   - Handle all relevant errors, exiting as appropriate and printing error message to stderr
    ./classifier -v -K 3 -d euclidean -p 100 training_data.bin testing_data.bin  
 */
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i < num_procs; i++){
        ...
        if (result < 0){
            ...
            int start_idx = i*N;
            if(i == num_procs - 1){
                N = test_set_size - start_idx;
            }
            if(close(fd[2*i][0]) == -1) {            
                perror("close");            
                exit(1);
            }   
            if(close(fd[2*i+1][1]) == -1) {            
                perror("close");            
                exit(1);
            }   
            
            if(write(fd[2*i][1], &start_idx, sizeof(int)) == -1) {            
                perror("write");            
                exit(1);
            }   
            if(write(fd[2*i][1], &N, sizeof(int)) == -1) {            
                perror("write");            
                exit(1);
            }   
            if(close(fd[2*i][1]) == -1) {            
            ...
            if(close(fd[2*i+1][0]) == -1) {            
                perror("close");            
                exit(1);
            }
            for(int k = 0; k < i; k++){
                if(close(fd[2*k + 1][0]) == -1){
                    perror("close");
                    exit(1);
                } 
            }       
            child_handler(training, testing, K, fptr, fd[2*i][0], fd[2*i+1][1]);
            exit(0);
        }
    }
    ...
    for(int i = 0; i < num_procs; i++){
        ...
        int count;
        if(read(fd[2*i+1][0], &count, sizeof(int)) == -1) {            
                perror("read");            
                exit(1);
            }   
        if(close(fd[2*i+1][0]) == -1) {            
