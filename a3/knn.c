...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    for(int i = start_idx; i < start_idx+N; i++){
        int p = knn_predict(training, &testing->images[i], K, fptr);
        if(p == testing->labels[i]){
            count += 1;
        }
    }
    if(write(p_out, &count, sizeof(int)) == -1) {            
        perror("write");            
        exit(1);
    }
    if(close(p_out) == -1) {            
...
double distance_cosine(Image *a, Image *b){
    ...
    double result = 2*acos(sum_ab/(sqrt(sum_a)*sqrt(sum_b)))/M_PI;
    return result;
}
