...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    usernames[user_index].username = malloc(sizeof(char)*(2 * BUF_SIZE + 2));
    int num_read = read(client_fd, usernames[user_index].username, sizeof(char)*(2 * BUF_SIZE + 2));
    if (num_read == 0){
...
int read_from(int client_index, struct sockname *usernames) {
    ...
    char full_buf[2 * BUF_SIZE + 2];
    char buf[BUF_SIZE + 1];
    char *username = usernames[client_index].username;
    
    strcpy(full_buf, username);
    ...
    strcat(full_buf, buf);
    int full_size = strlen(usernames[client_index].username) + 2 + num_read;
    full_buf[full_size] = '\0';
    ...
    for(int i = 0; i < MAX_CONNECTIONS; i++) {
        if(usernames[i].sock_fd != -1) {
            if(write(usernames[i].sock_fd, full_buf, strlen(full_buf)) != strlen(full_buf)) {
